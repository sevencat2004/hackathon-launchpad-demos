# Hackathon Launch Report

Status: first-stage started, not submitted, not paid.
Price: mixed; see each project entry.

## Ranked Tracks

### H0: Hack the Zero Stack

- Platform: Devpost
- Price: 80,000 USD cash prize pool
- Deadline: 2026-06-29 20:30 PDT
- Score: 74/100
- Recommendation: primary
- Reasons: strong agent/product fit; most demo assets can be prepared independently

### UiPath AgentHack

- Platform: Devpost / UiPath
- Price: 50,000 USD cash prize pool
- Deadline: 2026-06-29 17:00 EDT
- Score: 69/100
- Recommendation: primary-after-access
- Reasons: strong agent/product fit; requires user-owned platform/account steps

### Splunk Agentic Ops Challenge

- Platform: Devpost / Splunk
- Price: 20,000 USD total cash and feedback awards
- Deadline: 2026-06-15 23:45 PDT
- Score: 57/100
- Recommendation: narrow-mvp-only
- Reasons: strong agent/product fit; requires user-owned platform/account steps; short deadline; contest judging reduces payout certainty

### Sui Overflow 2026

- Platform: Sui ecosystem
- Price: 500,000+ USD total prize pool, track-dependent
- Deadline: 2026-06-21 hackathon phase
- Score: 56/100
- Recommendation: medium-term
- Reasons: requires user-owned platform/account steps; contest judging reduces payout certainty

## User Actions Needed

- Register/log in to Devpost for H0, UiPath, and Splunk.
- Confirm Vercel/AWS access for H0.
- Confirm UiPath platform access for UiPath AgentHack.
- Confirm Splunk account/trial access for Splunk.
- Confirm Sui wallet/account only if Sui track is actively pursued.
- User must complete final submissions and payment/KYC/tax steps.

